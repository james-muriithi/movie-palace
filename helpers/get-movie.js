...
let getMovie = async(id) => {
    const movieUrl = `${urls.API_URL}movie/${id}?&language=en-US&api_key=${urls.API_KEY}&append_to_response=videos,keywords`
    return axios.get(movieUrl)
        ...
        .then(res => {
            ...
            res.genres = gens
            if (country.length > 3) { country = country.slice(1, 4) }
